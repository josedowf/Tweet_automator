...
# returns the title, link, description and time posted of an article
# in the first news page for a Google search
def scraped_article(google_search):
    try:
        ...
        for item in soup.find_all('div', attrs={'class': 'Gx5Zad fP1Qef xpd EtOod pkphOe'}):
            ...
            data = open("news_from_google.csv", "a")
            data.write("{}, {}, {}, {}, {} \n".format(title, author, link, time, descript))
            data.close()
            return title, author, link, time, descript
    except Exception as e:
        print('An article could not be retrieved from Google')
        logging.exception(e)
        return 'Error'
